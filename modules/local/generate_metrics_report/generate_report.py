...
Reads staged inputs:
  plddt/   *.tsv  — single-model:   chain  residue  plddt
                  ...
                  — wide/AF3:       residue  ranked_0  ranked_1  ...  (*_plddt.tsv from AF3)
                                    Each column after 'residue' is a model; rows are AAs.
...
#   1. wide/AF3    — header: residue  ranked_0  ranked_1  ...
#                    detected when first column is "residue" and extra cols look like model names
#   2. allmodels   — header: model  chain  residue  plddt  (*_allmodels_plddt.tsv, Chai1)
#                    detected by "_allmodels" in filename
#   3. single      — header: chain  residue  plddt  (one model only)

data = {}   # data[sample][tool] = entry dict

def _rank_key_name(name):
...
for p in sorted(glob.glob("plddt/*.tsv")):
    ...
    # Detect wide format: first header key is "residue" and there are extra columns
    # that look like model names (contain "ranked", "model", or are numeric-suffixed)
    first_cols = list(rows[0].keys())
    is_wide = (
        first_cols[0] in ("residue", "positions")   # <-- accept "positions" too
        and len(first_cols) > 1
        and not is_multimodel
        and any(
            re.search(r'ranked|rank|model|\d$', c) for c in first_cols[1:]  # <-- accept "rank_N"
        )
    )
    ...
    if not resolved:
        for bn, sp in struct_files.items():
            if sample in bn:
                resolved = sp
                break
    if resolved:
    ...
    if is_wide:
        model_cols = first_cols[1:]
        print(f"[DEBUG] wide pLDDT for {sample}/{tool}: {len(rows)} residues, models={model_cols}")

        for col in model_cols:
            # Normalise: "rank_0" or bare "0" → "ranked_0"; "ranked_0" stays
            if re.match(r'^rank_(\d+)$', col):
                model_name = re.sub(r'^rank_', 'ranked_', col)   # rank_0 → ranked_0
            elif re.search(r'[a-z_]', col):
                model_name = col                                   # ranked_0, model_1, etc.
            else:
                model_name = f"ranked_{col}"                       # bare "0" → ranked_0

            plddt    = []
            residues = []
            for r in rows:
                try:
                    residues.append(int(r[first_cols[0]]))         # use actual first col name
                    plddt.append(float(r[col]))
                except (ValueError, KeyError):
                    pass
            if plddt:
                entry["models"][model_name] = {"plddt": plddt, "residues": residues}

        # Sort models by rank number and set ranked_0 as default
        sorted_models = sorted(entry["models"].keys(), key=_rank_key_name)
        best_name = sorted_models[0] if sorted_models else None
        if best_name:
            entry["plddt"]    = entry["models"][best_name]["plddt"]
            entry["residues"] = entry["models"][best_name]["residues"]

    elif is_multimodel:
        # Long/Chai1 allmodels format: model | chain | residue | plddt
        by_model = defaultdict(list)
        for r in rows:
            by_model[r.get("model", "ranked_0")].append(r)

        for model_name in sorted(by_model, key=_rank_key_name):
        ...
        # Single-model format: chain | residue | plddt
...
for p in sorted(glob.glob("scores/*.tsv")):
    ...
    if "model" in cols:
        ...
        ranked.sort(key=lambda rec: _rank_key_name(rec.get("model", "")))
